/* SQL (Structured Query Language)
   - 구조화된 질의 언어
   - RDBMS(관계DBMS)에서 저장된 데이터를 관리, 조작하거나 DB 구조를 제어할 때 사용하는 언어
...
# 메뉴 테이블에서 메뉴명, 가격 조회
select
    tbl_menu.menu_code, tbl_menu.menu_price
from
    tbl_menu;


## 조회되는 컬럼에 연산 가능
# 메뉴 테이블에서 메뉴명, 가격, 부가세(10%), 전체 가격(가격+부가세) 조회

select
    tbl_menu.menu_name,
    tbl_menu.menu_price,
    tbl_menu.menu_price * 0.1,
    tbl_menu.menu_price + tbl_menu.menu_price * 0.1
from
    tbl_menu;


# ===========================================================================
# ResultSet: SELECT 조회 결과를 모아놓은 행의 집합
# 컬럼명 별칭(as): ResultSet에 표기되는 컬럼명에 별치(alias) 부여 가능
# ===========================================================================

select
    tbl_menu.menu_name as 메뉴명,
    tbl_menu.menu_price as "메뉴 가격", # 쌍따옴표를 이용하여 띄어쓰기 된 칼럼명을 묶을 수 있음.
    tbl_menu.menu_price * 0.1 부가세,  # as 생략도 가능
    tbl_menu.menu_price + tbl_menu.menu_price * 0.1 "부가세 포함 가격"
from
    tbl_menu;
...
# 메뉴 테이블에서 메뉴명, 메뉴가격(원) 조회
select
    menu_name 메뉴명,
    concat('금 ',menu_price,'원') as "메뉴가격(원)"
from
    tbl_menu;
...
# - 특정 컬럼을 기준으로 삼아서 오름차순(ASC), 내림차순(DESC) 지정        # ASC: Ascending, DESC: Descending
...
from
    tbl_menu
order by
    menu_price asc; # table에서 특정 칼럼만 가져오는 경우에도 캐싱된 메모리에 이미 해당 table의 모든 칼럼 내용이 있기에
                    # 'menu_name'이 아닌 'menu_price'에 맞춰 order by도 할 수 있다.
...
# ===========================================================
# WHERE 행 필터링   (python의 filtering과 같은 기능을 제공)
...
# 반대 --> 10,000원 미만, 20,000원 초과
# 10,000원 이상, 20,000원 이하
...
select
    ...
    tbl_menu.category_code
from
    tbl_menu                                         # table에서 한 행씩 조회를 해서
where
#     category_code = 4                              # 행의 내용 중 이것이 있느냐를 조사
#    or
#     category_code = 6
#    or
#     category_code = 10
...
# like  (해당하는 자료 중 지정하는 것이 포함된 것을 찾게끔 하는 명령어)
# - 문자열 패턴 검사 연산자
# - 패턴을 나타내는 기호(와일드카드)로 %, _를 사용한다.
# '%': 포함, 0개 이상 포함된 것을 찾으라는 의미
# '_': 문자 1개 (한 칸) --> 원하는 문자 개수가 있는 걸 찾으라는 의미
select
    tbl_menu.menu_name
from
    tbl_menu
...
#     menu_name like '%밥';    # '밥'으로 끝나는 메뉴 조회           %가 뒤에 있는 것과 앞에 있는 것이 의미가 다름
...
# 비교 연산 '=' : 비교하는 두 값(data)이 같으면 True    (=동일한 값이 들어가 있으면)
...
from
    tbl_category
where
#     ref_category_code = null;   # 'null이라는 값'이 들어가 있는 것이 아니라 '그냥 아무 값이 없다는 뜻'이라 조건 불성립함.
#                                                                     # null != 값(data), just 컬럼이 비었다는 기호
    # =는 값을 대입, is는 상태를 의미
...
# ================================================================
# limit
# - 조회 결과(ResultSet)에서 지정된 크기만큼의 행만 조회 (1번행 ~ 지정된 행까지: index 번호)
...
select * from tbl_menu limit 5, 10; # 5행만큼 건너뛰고 6행부터 10개
